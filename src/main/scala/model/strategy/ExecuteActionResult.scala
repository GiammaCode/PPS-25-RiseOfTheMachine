...
object ExecuteActionResult:
  opaque type ExecuteActionResult[P <: PlayerEntity] = (P, Option[List[City]], List[String])

  /** Creates an ExecuteActionResult for a given player of type P and optional city. */
  def apply[P <: PlayerEntity](player: P, city: Option[List[City]], messages: List[String]): ExecuteActionResult[P] =
  ...
  extension [P <: PlayerEntity](res: ExecuteActionResult[P])
    ...
    def getCities: Option[List[City]] = res._2
