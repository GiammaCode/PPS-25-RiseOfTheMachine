...
type Target = String // In un sistema più robusto, questo potrebbe essere un tipo più specifico
...
object TurnAction {}
  ...
  def apply(
             executeLogic: List[Target] => Unit, // Modificato per accettare funzione che restituisce Unit
             actionTargets: List[Target] = List.empty
           ): TurnAction = new TurnAction {
    override def targets: List[Target] = actionTargets
