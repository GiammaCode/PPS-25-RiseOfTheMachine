...
class GameDifficultyTest extends AnyFunSuite with Matchers{
  ...
  test("AIStats are correctly returned for each difficulty") {
    aiStatsFor(Easy) shouldBe AIStats(infectionChance = 70, sabotagePower = 10)
    aiStatsFor(Normal) shouldBe AIStats(infectionChance = 50, sabotagePower = 5)
    aiStatsFor(Hard) shouldBe AIStats(infectionChance = 30, sabotagePower = 3)
  }
  ...
  test("Default AIStats fallback is Normal") {
    ...
    aiStatsFor(fakeDifficulty) shouldBe AIStats(infectionChance = 50, sabotagePower = 5)
  }
